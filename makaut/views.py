...
def upload_student(request):
    ...
    if hod.is_hod == "true":
        ...
        return message(title, error, request)
    return render(request, 'makaut/makaut_csv_upload.html', {'dept': hod.dept})


@login_required(login_url=reverse_lazy('login'))
def gen_certificate(request):
        # img = Image.open('media/makaut/cert.png/')
        # font = ImageFont.truetype('arial.ttf', 20)
        # font1 = ImageFont.truetype('arial.ttf', 16)
        # draw = ImageDraw.Draw(img)
        # draw.text(xy=(400, 250), text="SWARNAVA GANGULY", fill=(0, 0, 0), font=font)
        # draw.text(xy=(330, 295), text="43001017118", fill=(0, 0, 0), font=font)
        # draw.text(xy=(750, 295), text="2016-2020", fill=(0, 0, 0), font=font)
        # draw.text(xy=(320, 335), text="Computer Science Engineering", fill=(0, 0, 0), font=font)
        # draw.text(xy=(860, 425), text="100", fill=(0, 0, 0), font=font)
        # draw.text(xy=(140, 558), text="Jayanta Pal", fill=(89, 89, 89), font=font1)
        # draw.text(xy=(200, 579), text="CSE", fill=(89, 89, 89), font=font1)
        # draw.text(xy=(430, 560), text="U-ID: NIT/2016/0729", fill=(89, 89, 89), font=font)
        # img.show()
    return render(request, "makaut/certificate.html")
